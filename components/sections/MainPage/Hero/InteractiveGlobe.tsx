...
type GlobeGeometry = {
    ...
    accentAttribute: THREE.BufferAttribute;
    segmentCount: number;
};
...
function pointOnSphere(phi: number, theta: number, radius: number) {
...
function createGlobeGeometry(
    ...
    for (let latitude = 1; latitude < latitudeLines; latitude += 1) {
        ...
        for (let longitude = 0; longitude < longitudeLines; longitude += 1) {
            ...
            addSegment(
                pointOnSphere(phi, thetaStart, radius),
                pointOnSphere(phi, thetaEnd, radius),
    ...
    for (let longitude = 0; longitude < longitudeLines; longitude += 1) {
        ...
        for (let latitude = 0; latitude < latitudeLines; latitude += 1) {
            ...
            addSegment(
                pointOnSphere(phiStart, theta, radius),
                pointOnSphere(phiEnd, theta, radius),
...
export function InteractiveGlobe({
    ...
    useEffect(() => {
        ...
        function resize() {
            ...
            const pixelRatioLimit = window.innerWidth <= mobileBreakpoint ? 1.35 : 1.75;
